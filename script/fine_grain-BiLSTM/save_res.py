...
parser.add_argument('--trainset',
                    type=str,
                    default=os.path.join('data', 'fine_grain_trainset_0.csv'),
                    metavar='TRAINSET',
                    help='trainset path')
...
def main(args):
    ...
    model.load_state_dict(torch.load(f"./model_{args.idx}/best.model"))
    print(model)
    ...
    path = f'../fine-grained_evaluation/res/fine_grain_testset_{args.idx}.csv'
    res_df = pd.read_csv(path)
    res_df.insert(res_df.shape[1], "#Bi-LSTM", preds)
